...
import com.kitever.pos.fragment.PosCustomerListFragment;
...
public class POSCustomerAdapterFragment extends BaseAdapter implements Filterable {
...
//	TextView name, order, business,
    ...
    private PosCustomerListFragment context;
    private String selectedItemType;
    private POSCustomerAdapterFragment.ItemFilter itemFilter = new POSCustomerAdapterFragment.ItemFilter();
    private TotalRows rows;
    public POSCustomerAdapterFragment(ArrayList<CustomerList> posCustomerList, PosCustomerListFragment context)
    {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        // TODO Auto-generated method stub
        POSCustomerAdapterFragment.ViewHolder holder;
        if (convertView == null) {
            holder = new POSCustomerAdapterFragment.ViewHolder();
            LayoutInflater inflater = LayoutInflater.from(parent.getContext());
            convertView = inflater.inflate(
                    R.layout.pos_customer_adapter_layout, parent, false);

            holder.customer_adapter_layout=(LinearLayout) convertView.findViewById(R.id.customer_adapter_layout);
            holder.name=(TextView) convertView.findViewById(R.id.pos_customer_name);
            ...
            setRobotoThinFont(holder.name,context.getActivity());
            setRobotoThinFont(holder.order,context.getActivity());
            setRobotoThinFont(holder.business,context.getActivity());
            setRobotoThinFont(holder.credit,context.getActivity());
            setRobotoThinFont(holder.lastDate,context.getActivity());
            setRobotoThinFont(holder.lastMonth,context.getActivity());

            holder.name.setTextColor(Color.parseColor(CustomStyle.THEME_FONT_COLOR));
            holder.order.setTextColor(Color.parseColor(CustomStyle.THEME_FONT_COLOR));
            holder.business.setTextColor(Color.parseColor(CustomStyle.THEME_FONT_COLOR));
            holder.credit.setTextColor(Color.parseColor(CustomStyle.THEME_FONT_COLOR));
            holder.lastDate.setTextColor(Color.parseColor(CustomStyle.THEME_FONT_COLOR));
            holder.lastMonth.setTextColor(Color.parseColor(CustomStyle.THEME_FONT_COLOR));

            holder.customer_adapter_layout.setBackgroundColor(Color.parseColor(CustomStyle.LISTVIEW_BACKGROUND_ENABLED));

            convertView.setTag(holder);
        } else {
            holder = (POSCustomerAdapterFragment.ViewHolder) convertView.getTag();
        }
...
//				 orderDate = list.get(i).getLastOrderDate();
    ...
    private static class ViewHolder{
        ...
        LinearLayout customer_adapter_layout;
    }
