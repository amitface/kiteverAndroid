...
import sms19.listview.newproject.MerchatStorePackage.Adapter.MerchantCartOrderAdapter;

import static com.kitever.app.context.CustomStyle.setRobotoThinFont;
import static com.kitever.utils.Utils.actionBarSettingWithBack;

public class MerchantCartOrderList extends AppCompatActivity implements NetworkManager, TotalRows, View.OnClickListener, DatePickerDialog.OnDateSetListener,MerchantCartOrderAdapter.Action {
    ...
    private MerchantCartOrderAdapter orderAdapter;
    ...
    private FrameLayout checkbox_layout;
    SparseBooleanArray checkBoxState = new SparseBooleanArray();

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_merchant_cart_order_list);
        actionBarSettingWithBack(this, getSupportActionBar(), "Customer Orders");
        context = this;
        setScreen();
        fetchOrderList();
    }
    ...
    private void setScreen() {
        ...
        selectall = (CheckBox) findViewById(R.id.selectall_checkbox);

        checkbox_layout=(FrameLayout)findViewById(R.id.checkbox_layout);
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                try
                {
                    for(int i=0; i < orderListView.getCount(); i++){
                        checkBoxState.put(i, isChecked);
                       OrderDetailModelData bean = orderList.get(i);
                        bean.setSelected(isChecked);
                    }
                    orderAdapter.notifyDataSetChanged();
                }
                catch(Exception e){Log.i("my Error ","Eror:"+e.getMessage());}
            }
        });*/

      //  int states[][] = {{android.R.attr.state_checked}, {}};
      //  int colors[] = {R.color.black, R.color.green_home};
      //  CompoundButtonCompat.setButtonTintList(selectall, new ColorStateList(states, colors));

        selectall.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
               // if(selectall.isChecked()) selectall.setChecked(true);
                //else selectall.setChecked(false);
                try
                {
                    for(int i=0; i < orderListView.getCount(); i++){
                        checkBoxState.put(i, selectall.isChecked());
                        OrderDetailModelData bean = orderList.get(i);
                        bean.setSelected(selectall.isChecked());
                    }
                    orderAdapter.notifyDataSetChanged();
                }
                catch(Exception e){Log.i("my Error ","Eror:"+e.getMessage());}
        ...
        if (response != null && response.length() > 0) {
            if (requestId == KEY_FETCH_ORDER_DETAILS) {
                ...
                if (orderList != null && orderList.size() > 0) {
                    orderAdapter = new MerchantCartOrderAdapter(this, orderList,selectall,checkBoxState);
