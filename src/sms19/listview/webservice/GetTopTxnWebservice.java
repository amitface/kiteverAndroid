...
import sms19.listview.newproject.model.getTopTransactionBalance;
...
public class GetTopTxnWebservice {

	// -------------------------------------Global variable-----------------------------
	
    public static final String EQU      = "=";
	public static final String AND      = "&";
    //public static final String BSE_URL    ="http://sms19.com/NewService.aspx?Page=";
//	public static final String BSE_URL    ="http://kit19.com/NewService.aspx?Page=";
	...
	public  static final int TYPE_FETCH_TOP_TXN     = 80;//Fetch_Message  for thread                      --80

	
	// Use listener in this class for taking response from server service side
	private static ServiceHitListenerBalance listener;
	...
	public GetTopTxnWebservice(HttpPost HttpPost, String url, int type, int ServiceName, ServiceHitListenerBalance listener)
	{
		
		// pass value from current class to this handler global variable value.
		GetTopTxnWebservice.listener=listener;
	 ...
	 //Get Top Current balance --80
	   public static class getCurrentBalance
	   {
		   //http://sms19.com/NewService.aspx?Page=GetUserTopTransaction&Userid=19762
		   ...
		   public static String geturl(String userid)
		   {
			   ...
			   return Apiurls.KIT19_BASE_URL + SERVICE + AND + UserID+ EQU+ userid;
	...
	public class AsyncExecuter extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) 
		{
			...
			try {
				//Log.w("TAG", "Response: " + strbuild.toString());
		...
		protected void onPostExecute(String result) {
			...
			else
			{
			   Gson g = new Gson();
				...
				//fetch message thread --80
			  if(SERVICENAME == TYPE_FETCH_TOP_TXN)
				 {
					 try{
						 listener.onSuccess(g.fromJson(result,getTopTransactionBalance.class), SERVICENAME);
					 }
					 catch(Exception e){
						 e.printStackTrace();
						 listener.onError("Please Try Again Later", SERVICENAME);
	...
	public interface ServiceHitListenerBalance
	{
